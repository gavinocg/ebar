...
- **Clean-migration / punto de retorno:** `php artisan clean-transactional --solo-superadmin` vacía **toda** la base (transaccional + bares, sucursales, membresías, impresoras, configuración, roles personalizados y usuarios), dejando **únicamente el acceso `super_admin`** y la estructura/migraciones. Sirve para volver a este punto base con solo el superadmin. Avisa y devuelve `FAILURE` si no queda ningún `super_admin`.
